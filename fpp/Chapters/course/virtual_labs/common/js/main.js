...
function main() {
  ...
  loader = new createjs.LoadQueue(
    false,
    scaledValue(1) == 1 ? resources.basePath : resources.basePathDevice
  );
...
function createLayout(array, parent, bool) {
  ...
  array.forEach(function (object) {
    ...
    if (object.hitBounds) {
      child._hitBounds = new createjs.Rectangle(
        object.hitBounds.x,
        object.hitBounds.y,
        object.hitBounds.width,
        object.hitBounds.height
      );
    }
      ...
      if (scaledValue.scalableProps.indexOf(i) != -1) {
        if (i == "font") {
          ...
          if (!isNaN(parseInt(arr[0]))) {
            ...
            // bold 10px Arial
            object.props[i] =
              arr[0] + " " + scaledValue(parseInt(arr[1])) + "px " + arr[2];
          }
