var GasPart = (function () {
  ...
  GasPart.prototype.collide = function (molecule) {
    ...
    var V = new Vector(
      Model.pixelToPoint(this.velocity.x),
      Model.pixelToPoint(this.velocity.y),
    );
    var oM = molecule.mass;
    var oP = new Vector(molecule.position.x, molecule.position.y);
    var oV = new Vector(
      Model.pixelToPoint(molecule.velocity.x),
      Model.pixelToPoint(molecule.velocity.y),
    );
  ...
  GasPart.prototype.updateVelFromSpeed = function () {
    ...
    V.y = this.m_vyp;
    this.m_theta =
      V.magnitude() > 0 ? V.angle(true) : Math.random() * 2 * Math.PI;
