...
var Button = (function () {
  function Button(o) {
    ...
    if (!o.up || !o.down || !o.over) {
      console.error(
        "Please provide image names for up, over and down state for button - " +
          o.name,
      );
    }
...
var NumericStepper = (function () {
  function NumericStepper(o) {
    ...
    this._value = o.hasOwnProperty("value") ? o.value : this._min;
    ...
    if (o.horizontal) {
      if (this._inc.getBounds().width) {
        this._inc.x =
          this._dec.getBounds().width + (o.hasOwnProperty("gap") ? o.gap : 5);
      } else {
        this._inc.x =
          this._dec.children[0].image.naturalWidth +
          (o.hasOwnProperty("gap") ? o.gap : 5);
      }
    } else {
      if (this._inc.getBounds().height) {
        this._dec.y =
          this._inc.getBounds().height + (o.hasOwnProperty("gap") ? o.gap : 5);
      } else {
        this._dec.y =
          this._inc.children[0].image.naturalHeight +
          (o.hasOwnProperty("gap") ? o.gap : 5);
      }
    }
...
var Slider = (function () {
  function Slider(bmp, min, max, minStep, maxStep, shape, object, prop) {
    ...
    hitArea.graphics
      ...
      .rect(
        -5,
        -5,
        this.bmp.getBounds().width + 10,
        this.bmp.getBounds().height + 10,
      )
      .endFill();
  ...
  Slider.prototype = {
    ...
    _drag: function (e) {
      ...
      this.bmp[this.prop] = Math.round(
        Math.max(this.minX, Math.min(p[this.prop], this.maxX)),
      );
    ...
    _updateLine: function () {
      if (this.shape) {
        if (this.prop == "x") {
          this.shape.graphics
            .clear()
            .setStrokeStyle(this.shapeData.lineWidth)
            .beginStroke(this.shapeData.color)
            .moveTo(
              this.shapeData.direction == "left" ? this.minX : this.maxX,
              this.bmp.y,
            )
            .lineTo(this.bmp.x, this.bmp.y)
            .endStroke();
        } else {
          this.shape.graphics
            .clear()
            ...
            .moveTo(
              this.bmp.x,
              this.shapeData.direction == "top" ? this.minX : this.maxX,
            )
            .lineTo(this.bmp.x, this.bmp.y)
            .endStroke();
        }
  ...
  Object.defineProperty(Slider.prototype, "value", {
    get: function () {
      var value =
        ((this.bmp[this.prop] - this.minX) / (this.maxX - this.minX)) *
    ...
    set: function (value) {
      ...
      var x =
        this.minX +
        (value * (this.maxX - this.minX)) / (this.maxStep - this.minStep);
...
var SSText = (function () {
  ...
  SSText.prototype._refreshPosition = function () {
    ...
    this._subText.x = this._supText.x =
      this._mainText.getMeasuredWidth() + this._hgap;
  ...
  Object.defineProperty(SpeedoMeter.prototype, "value", {
    ...
    set: function (value) {
      ...
      if (value >= 0) {
        ...
        this.needle.rotation =
          this._minAngle +
          (this._value * (this._maxAngle - this._minAngle)) /
            (this._maxValue - this._minValue);
      } else {
        this._value = Math.max(
          this._minValue,
          Math.min(this._maxValue, Math.abs(value)),
        );
        this.needle.rotation =
          this._minAngle -
          (this._value * (this._maxAngle - this._minAngle)) /
      ...
      if (this.valueText) {
        if (value >= 0) {
          this.valueText.text = this._value.toFixed(1);
        } else {
...
var VectorLine = (function () {
  function VectorLine(
    ...
    this._snapValue = _snapValue ? _snapValue : 1;
  ...
  VectorLine.prototype = {
    ...
    _drag: function (e) {
      var p = new Vector(
        e.stageX / this._bmp.stage.scaleX - this._baseVector.x,
        e.stageY / this._bmp.stage.scaleY - this._baseVector.y,
      );
      var prevAngle = this.angle;
      this.magnitude = p.length() / this._unit;
      var angle = p.angle(); //.toFixed(this.toFixed) * 1;
      angle = angle;
      var round = Math.floor(angle);
      var decimal = angle - round;
      if (this.restriction) {
        //console.log(p.length() <= (this._unit)*this.magnitude, p, p.length(),(this._unit),this.magnitude);
        if (p.length() <= this._unit * this.magnitude) {
          // console.log(p.length(), (this._unit)*this.magnitude);
          if (this._snapValue >= 1) {
          } else if (this._snapValue >= 0) {
            decimal =
              this._snapValue < decimal
                ? decimal - (decimal % this._snapValue)
                : 0;
            angle = round + decimal;
          }
        ...
        // console.log(p.length(), (this._unit+0.25)*this.magnitude);
        if (this._snapValue >= 1) {
        } else if (this._snapValue >= 0) {
          decimal =
            this._snapValue < decimal
    ...
    _updateLine: function () {
      ...
      if (this._shape && this._shape.graphics) {
        this._shape.graphics
          .clear()
          .setStrokeStyle(this._shapeData.lineWidth)
          .beginStroke(this._shapeData.color)
          .moveTo(this._baseVector.x, this._baseVector.y)
          .lineTo(p.x, p.y)
          .endStroke();
      }
      this._bmp.x = p.x;
  ...
  Object.defineProperty(VectorLine.prototype, "magnitude", {
    ...
    set: function (value) {
      value =
        Math.max(this._minMagnitude, Math.min(this._maxMagnitude, value)) *
        this._unit;
