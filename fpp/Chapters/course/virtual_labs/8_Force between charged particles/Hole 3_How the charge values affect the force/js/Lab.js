...
var obstacleArray = [
  { left: 625, right: 645, top: 21, bottom: 361 },
  { left: 21, right: 386, top: 295, bottom: 317 },
];
var sandTrap = [
  { left: 386, right: 442, top: 230, bottom: 283 },
  { left: 586, right: 642, top: 404, bottom: 470 },
];
var scores = {
  score1: 0,
  score2: 0,
...
var Lab = (function () {
  ...
  function updateTable() {
    this.layout.score3.text = this.putCount;
    this.layout.total.text = this.total;

    this.temp = -(6 - this.putCount);
    this.temp2 = -(this.totalDiff - this.putCount);

    if (this.temp < 0) {
      this.layout.diff3.color = "green";
      this.layout.diff3.text = this.temp;
    } else {
      this.layout.diff3.color = "red";
      this.layout.diff3.text = "+" + this.temp;
    }

    if (this.temp2 < 0) {
      this.layout.totalDiff.color = "green";
      this.layout.totalDiff.text = this.temp2;
    } else {
      this.layout.totalDiff.color = "red";
      this.layout.totalDiff.text = "+" + this.temp2;
    }

    scores["score3"] = this.putCount;
    diff["diff3"] = this.temp;
  }
  ...
  Activity.prototype.reset = function () {
    ...
    this.layout.distance.text = "0.294";
    this._allCharges.putter.reset();
    this._allCharges.ball.reset();
    createjs.Tween.get(this.layout.ball, { override: true }).to(
    ...
    this.layout.score3.text = 0;
    this.layout.diff3.color = "black";
    this.layout.diff3.text = "--";
    this.putCount = 0;
    this._onBallStop();
  };

  Activity.prototype._fetchData = function () {
    if (location.search.length <= 1) {
      return;
    }
    var s = location.search.split("&");
    var s1 = JSON.parse(s[0].split("%22").join('"').substring(1));
    var s2 = JSON.parse(s[1].split("%22").join('"'));
    for (var i in s1) {
      scores[i] = s1[i];
    }
  ...
  Activity.prototype._enterFrame = function () {
    this._allCharges.putter.xRadius = Maths.getDistance(
      this._allCharges.putter,
      this._allCharges.ball,
    );
  ...
  Activity.prototype._updatePutter = function (e) {
    this._allCharges.putter.updateRotation(
      this._allCharges.ball,
      this._ballRadius /*  + 1.35 */,
    );
    this._allCharges.putter.xRadius = Maths.getDistance(
      this._allCharges.putter,
      this._allCharges.ball,
  ...
  Activity.prototype._createInstances = function () {
    ...
    this._allCharges.putter /* .enable() */
      .addEventListener("moved", this._updatePutter);
...
var ChargeClass = (function () {
  var Class = function (o) {
    this._initData = {
      ...
      hitBounds: o.displayObject._hitBounds
        ? o.displayObject._hitBounds.clone()
  ...
  Class.prototype._Hole = function (_charges) {
    ...
    lab.layout.ball.x = 920;
    lab.layout.ball.y = 546;
    createjs.Tween.get(lab.layout.ball).to(
  ...
  Class.prototype._update = function (_charges, count) {
    if (this.started) {
      ...
      if (
        lab.layout.ball.x < 935 &&
        lab.layout.ball.x > 903 &&
        lab.layout.ball.y < 562 &&
        lab.layout.ball.y > 528 &&
  ...
  Class.prototype._moveObject = function (_charges) {
    lab.layout.distance.text = (
      Maths.getDistance(lab.layout.ball, lab.layout.putter) / lab._unit
    ).toFixed(3);
    var speed,
      Fc = this._Fc,
      Ff = this._Ff,
    ...
    if (this.type == TYPE_BALL) {
      ...
      if (
        (speed < 7 && this.time - startTime > 3) ||
        this.time - startTime > 5
      ) {
        this._velocity.set(0, 0);
        this._stopped = true;
      ...
      if (
        _charges[0].charge < 0 &&
        Maths.getDistance(_charges[0], this) / this._unit < 0.1
      ) {
        this._velocity.set(0, 0);
        this.x =
          _charges[0].x +
          Math.cos(_charges[0].rotation * Maths.TO_RADIANS) * 0.03;
        this.y =
          _charges[0].y +
          Math.sin(_charges[0].rotation * Maths.TO_RADIANS) * 0.03;
        lab.layout.playButton.enable();
        if (
          lab.layout.ball.x < 935 &&
          lab.layout.ball.x > 903 &&
      ...
      if (Math.abs(m_vxp0) > 0.001 || Math.abs(m_vyp0) > 0.001) {
        ...
        Ff.set(
          Math.cos(fAng) * this.friction * -this.gravity,
          Math.sin(fAng) * this.friction * -this.gravity,
        );
        if (Ff.x == -this._xF && Math.abs(m_vxp0) < /* m_swnd.xpFromXw */ 5) {
          //if the force of friction is the exact opposite from the previous cycle, it indicates that the particle has changed direction due to the force of friction
          Ff.x = 0;
          this._velocity.x = 0;
        }
      ...
      // skipped
      if (this.x > 390 && this.x < 436 && this.y > 230 && this.y < 279) {
        Vector.multiply(Ff, 5, Ff);
      } else if (this.x > 590 && this.x < 640 && this.y > 405 && this.y < 465) {
        Vector.multiply(Ff, 5, Ff);
      }
      ...
      if (Fc.x != 0) {
        if (Ff.x / Fc.x < -1.0) {
          //the only force acting on the ball is friction
          if (m_vxp0 / (m_vxp0 + Fnet.x * this._timeStep) <= 0) {
            //did it switch directions?
            this._velocity.set(0, 0);
            this._stopped = true;
            lab.layout.playButton.enable();
          }
        }
      } else if (m_vxp0 + (Fc.x + Ff.x) * this._timeStep != 0) {
        //the only force acting on the ball is friction
        if (m_vxp0 / (m_vxp0 + (Fc.x + Ff.x) * this._timeStep) <= 0) {
          //did it switch directions?
          this._velocity.set(0, 0);
          this._stopped = true;
          lab.layout.playButton.enable();
  ...
  Class.prototype._findAccelVecs = function (_charges) {
    ...
    Fc.set(0, 0, 0);
    var len = _charges.length;

    if (this._intersects(sandTrap[0]) || this._intersects(sandTrap[1])) {
      len = _charges.length - sandTrap.length;
    }
    //what is the acceleration vector for all objects acting on me?
    for (var iso = 0; iso < len; iso++) {
      soTemp = _charges[iso];
      ...
      a =
        (k * this.charge * soTemp.charge) /
        Math.pow(vecLength, 2) /
        this.protonMass;
  ...
  Class.prototype.updateRotation = function (_charge, _length) {
    ...
    var angle = Maths.getAngle(_charge, this).toFixed(1) * 1;
    this.rotation = angle;
    if (dist <= _length) {
      var _p = Maths.getPoint(_charge, angle * Maths.TO_RADIANS, _length - 8);
      this.x = _p.x;
      this.y = _p.y;
    }
    return this;
  ...
  Class.prototype._obstacleBounce = function () {
    var temp;
    var xDist,
      yDist,
      xDir,
    ...
    if (obstacleArray != null) {
      for (var i = 0; i < obstacleArray.length; i++) {
        temp = obstacleArray[i];

        if (this._intersects(temp)) {
          if (this._velocity.x > 0) {
            xDist = this.x + lab._ballRadius - temp.left;
            xDir = -1;
          } else {
            xDist = this.x - lab._ballRadius - temp.right;
            xDir = 1;
          }

          if (this._velocity.y > 0) {
            yDist = this.y + lab._ballRadius - temp.top;
            yDir = 1;
          } else {
            yDist = this.y - lab._ballRadius - temp.bottom;
            yDir = -1;
          }
          ...
          if (pLR == 1) {
            this._velocity.x = Math.abs(this._velocity.x);
            this.x = temp.right + lab._ballRadius;
          } else if (pLR == -1) {
            this._velocity.x = Math.abs(this._velocity.x) * -1;
            this.x = temp.left - lab._ballRadius;
          }

          if (pTB == 1) {
            this._velocity.y = Math.abs(this._velocity.y) * -1;
            this.y = temp.top - lab._ballRadius;
          } else if (pTB == -1) {
            this._velocity.y = Math.abs(this._velocity.y);
            this.y = temp.bottom + lab._ballRadius;
          }
        } else {
          pLR = 0;
          pTB = 0;
  ...
  Class.prototype._intersects = function (obs) {
    if (
      this.x + lab._ballRadius > obs.left &&
      this.x - lab._ballRadius < obs.right &&
      this.y + lab._ballRadius > obs.top &&
      this.y - lab._ballRadius < obs.bottom
    ) {
      return true;
    } else {
      return false;
  ...
  Class.prototype._click = function () {
    scores["total"] = lab.total;
    diff["totalDiff"] = lab.temp2;
    var a = JSON.stringify(scores);
    var b = JSON.stringify(diff);
    window.location.href =
      "../Hole 4_Extra credit − Determining Coulomb’s law/index.html?" +
      a +
      "&" +
      b;
  };
  ...
  Class.prototype._updateCollisionArea = function () {
    ...
    if (this._rect) {
      ...
      this._rectPoints.forEach(function (p, i) {
        Vector.components(
          this._rect[i].magnitude(),
          this._rect[i].angle(true) + this.rotation * Maths.TO_RADIANS,
          p,
        );
        Vector.add(this, p, p);
        this._rectShape.graphics[i == 0 ? "moveTo" : "lineTo"](p.x, p.y);
      ...
      this._rectShape.graphics
        .lineTo(this._rectPoints[0].x, this._rectPoints[0].y)
        .endStroke();
    }
    if (this._circle) {
      this._rectShape.graphics
  ...
  Object.defineProperty(Class.prototype, "frictionalForce", {
    get: function () {
      return 0.4 * this.protonMass * this.gravity;
    },
  });
