var Lab = (function () {
  ...
  Activity.prototype.startLab = function () {
    ...
    scope.layout.stepper2.value = 0.1;
    ...
    this.vector = new VectorLine(
      this.arrow,
      { x: this.layout.ball.x, y: this.layout.ball.y },
      { min: -1000, max: 2000 },
      { min: 0, max: 360 },
      this.arrowline,
      { color: "black" },
  ...
  Activity.prototype._vectorMag = function () {
    this.temp =
      this.layout.stepper1.value / this.layout.stepper2.value / this._unit;
  ...
  Activity.prototype._play = function () {
    ...
    this.force =
      (this.K * this._putterCharge * this.protonCharge * this.protonCharge) /
      Math.pow(this.ballPutterDist, 2);
    ...
    if (!(this.displacement <= this.layout.putter.x + 5)) {
      createjs.Tween.get(this.layout.ball, { override: true })
        .to({ x: this.layout.ball.x + this.dist }, 1000, createjs.Ease.quadOut)
        .call(this._checkHole);
    } else {
      createjs.Tween.get(this.layout.ball, { override: true }).to(
        { x: this.layout.putter.x + 5 },
        300,
  ...
  Activity.prototype._checkHole = function () {
    if (this.layout.ball.x < 766 && this.layout.ball.x > 740) {
      ...
      createjs.Tween.get(this.layout.ball).to(
