var Lab = (function () {
  ...
  Activity.prototype.startLab = function () {
    ...
    this._objects.forEach(function (name, i) {
      ...
      var arrow = new VectorLine(
        b,
        { x: this.layout[name].regX, y: this.layout[name].regY },
        { min: 0, max: 100 },
        { min: 0, max: 360 },
        s,
        data[i],
  ...
  function enterFrame() {
    if (
      this.layout.blueBall.x > this.OUTOFPP ||
      this.layout.blueBall.y > this.OUTOFPP ||
      this.layout.brownBall.x > this.OUTOFPP ||
      this.layout.brownBall.y > this.OUTOFPP
    ) {
      Ticker.remove(this._enterFrame);
    ...
    this._calculateForces();
    var factor =
      (Math.abs(this.layout.blueBall._charge) +
        Math.abs(this.layout.brownBall._charge)) /
      2;
    this._objects.forEach(function (name, i) {
    ...
    if (checkCollision.call(this)) {
      ...
      var ball1 = this.layout.blueBall,
  ...
  function pressMove(e) {
    Ticker.remove(this._play);
    if (
      e.currentTarget == this.layout.blueBall._arrow._bmp ||
      e.currentTarget == this.layout.brownBall._arrow._bmp
    ) {
      return;
    }
    ...
    for (; i < this._objects.length; i++) {
      ...
      for (var j = 0; j < this._objects.length; j++) {
        if (i != j) {
          ...
          var dToFixed = d.toFixed(Math.abs(distance2.power) + 2);
          //console.log(dToFixed);

          if (object._charge * object2._charge == 0) {
            forceValue = 0;
          } else {
            forceValue =
              (KE * object2._charge * object._charge) / Math.pow(dToFixed, 2);
  ...
  function forceDisplay1() {
    if (this.layout.blueBall.x > this.RIGHTLIMIT) {
      ...
      rectangle1.graphics
        .beginFill("#ffffff")
        .drawRect(
          this.layout.fdisplay1.x - 16,
          this.layout.fdisplay1.y - 10,
          137,
          37,
        );
    } else {
  ...
  function forceDisplay2() {
    if (this.layout.brownBall.x > this.RIGHTLIMIT) {
      ...
      rectangle2.graphics
        .beginFill("#ffffff")
        .drawRect(
          this.layout.fdisplay2.x - 16,
          this.layout.fdisplay2.y - 10,
          137,
          37,
        );
    } else {
  ...
  Activity.prototype.reset = function () {
    ...
    Ticker.remove(this._enterFrame);
    Ticker.remove(this._play);
    this.layout.blueBall.x = 191;
    this.layout.blueBall.y = 351;
    this.layout.brownBall.x = 831;
    this.layout.brownBall.y = 351;

    this.layout.blueBall._arrow._baseVector.x = 191;
    this.layout.blueBall._arrow._baseVector.y = 351;
    this.layout.brownBall._arrow._baseVector.x = 831;
    this.layout.brownBall._arrow._baseVector.y = 351;
    ...
    this.layout.distanceop1.text = "1.00 × 10";
    this.layout.distanceop2.text = "-6";
