  ...
  function enterFrame() {
    ...
    for (var ll = 0; ll < 2; ll++) {
      ...
      if (pAtStart) {
        ...
        this._thetaAtStart =
          2 * Math.PI * this._F * 0.7 * (pAtStart.x + this._phase);
      }

      if (this._thetaAtStart <= -2 * Math.PI) {
        /*console.log("complete---->"); */ this._waveStart += 3;
        ...
        if (
          this._waveStart >= this._points.length - 1 &&
          this._flagOnce == true
        ) {
          this._first = true;
          this.layout.pauseButton.visible = false;
      ...
      for (; i < this._waveEnd; i++) {
        ...
        if (p) {
          p.y =
            this._A *
            Math.sin(2 * Math.PI * this._F * 0.7 * (p.x + this._phase));
      ...
      for (i = 0; i < this._waveStart; i++) {
        ...
        if (p && this._points[this._waveStart]) {
          p.y =
            (Math.exp(p.x * 10) - 1) * (this._points[this._waveStart].y / 12);
      ...
      this._points.forEach(function (v, i) {
        ...
        if (v.bird) {
          ...
          if (
            v.bird._thresholdA &&
            v.y.toFixed(2) * 1 <= -(v.bird._thresholdA - 0.01) &&
            this._A != 0 &&
            v.bird._thresholdA <= this._A
          ) {
            v.bird._flew = true;
          }
