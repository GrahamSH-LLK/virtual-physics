    ...
    for (var ll = 0; ll < 2; ll++) {
      ...
      if (pAtStart) {
        this._thetaAtStart =
          2 *
          Math.PI *
          this._FreqArray[this._FreqArray.length - 1] *
          1.25 *
          (pAtStart.x + this._phaseArray[this._phaseArray.length - 1]);
      }
      ...
      if (this._thetaAtStart <= -1 * num * Math.PI && this._first == false) {
        ...
        if (cnt == 1) {
          waveLength =
            this._points[this._waveEnd - 1].x -
            this._points[waveStartArray[0]].x;
          speed = waveLength * this._FreqArray[0];
          speed = 0.8;
        } else {
          //waveLength = this._points[waveStartArray[cnt-2]].x-this._points[waveStartArray[cnt-1]].x;
          waveLength = speed / this._FreqArray[cnt];
        }
        //console.log("cnt---"+cnt+"----waveLength---"+waveLength);
        cnt++;
      }

      for (n = 1; n < cnt; n++) {
        waveStartArray[n - 1] += 3;
        waveStartArray[n - 1] = Math.min(
          this._points.length,
          waveStartArray[n - 1],
        );
      ...
      for (i = 0; i < this._wInitial; i++) {
        ...
        if (p) {
          p.y =
            (Math.exp(p.x * 10) - 1) * (this._points[this._wInitial].y / 12);
        }
      }
      for (n = 0; n < cnt; n++) {
      ...
      this.layout.waveshape.graphics
        .clear()
        .setStrokeStyle(3)
        .beginStroke("#000")
        .moveTo(0, 0);
        ...
        if (v.bird) {
          if (!v.bird._flew) {
            ...
            if (v.bird._wingFlip) {
              ...
              this.layout.leftleg.x =
                v.x * this.unit - 7 + this.layout.birdscontainer.x;
              this.layout.leftleg.y =
                v.y * this.unit - 7 + this.layout.birdscontainer.y;
              this.layout.rightleg.x =
                v.x * this.unit + 7 + this.layout.birdscontainer.x;
              this.layout.rightleg.y =
                v.y * this.unit - 7 + this.layout.birdscontainer.y;
            }
            if (v.bird._tilting) {
          ...
          if (
            waveLength &&
            v.bird._thresholdF &&
            v.y.toFixed(2) * 1 >= this._A - 0.01 &&
            this._A != 0 &&
            (waveLength.toFixed(1) == 1 ||
              waveLength.toFixed(2) == 0.5 ||
              waveLength.toFixed(2) == 0.33 ||
              waveLength.toFixed(2) == 0.25 ||
              waveLength.toFixed(3) == 0.2 ||
              waveLength.toFixed(3) == 0.167)
          ) {
            v.bird.flag1 = true;
          }
          if (
          ...
          if (
            this.layout["bird1"].flag1 &&
            this.layout["bird2"].flag1 &&
            this.layout["bird3"].flag1
          ) {
            if (v.bird._thresholdF) {
              //console.log("hiii i am  here"+v.y.toFixed(2));
              //console.log("rotAng---->"+rotAng);
              if (v.y.toFixed(1) * 1 == 0) {
                this._cntFlew++;
                //console.log("this._cntFlew--->"+this._cntFlew);
                if (this._cntFlew >= 50) {
                  this.layout["bird1"]._flew = true;
                  this.layout["bird2"]._flew = true;
                  this.layout["bird3"]._flew = true;
                }
                //Ticker.remove(this.enterFrame);
              }
            }
          ...
          if (v.bird._wingFlip) {
            ...
            if (v.bird._previousYPos > v.bird.y) {
              ...
              if (this._first) {
                if (this._once == false) {
                  v.bird.gotoAndPlay(v.bird._headAnimFrame);
                  this._once = true;
                }
