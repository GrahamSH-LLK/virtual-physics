var Lab = (function () {
  ...
  Activity.prototype.startLab = function () {
    ...
    for (j = 2; j <= 8; j++) {
      Model.hand.xPoints.push(
        (Model.hand.x.max - Model.hand.x.min) / j + Model.hand.x.min,
      );
    }
    ...
    for (j = 2; j <= 7; j++) {
      Model.hand.xCheckPoints.push(
        ((Model.hand.x.max - Model.hand.x.min) / j +
          Model.hand.x.min +
          (Model.hand.x.max - Model.hand.x.min) / (j + 1) +
          Model.hand.x.min) /
          2,
      );
    }
  ...
  function playClicked(e) {
    ...
    //	}
    if (Model.playclicked == false) Model.playclicked = true;
    Model.currentFrequency = Model.frequency * Model.currentId;
    lab.layout.frequencyValue.text = Model.currentFrequency;
    lab.layout.note.text = Model.waveNotes["wave_" + Model.currentId];
    playAnimation();
  ...
  function _tick() {
    ...
    draw(
      ...
      ((Model.time * 100) % 32) / Model.currentId,
    );
  }
