var Lab = (function () {
      ...
      for (j = 1; j <= 17; j++) {
        if (i <= Model.waveLength["wave_" + j]) {
          Model.wavePoints["wave_" + j].push({
            ...
            y:
              Model.amplitude *
              Math.sin((Model.phasediff * i) / Model.waveLength["wave_" + j]),
          });
        }
      }
    }
  ...
  Activity.prototype.reset = function () {
    ...
    if (Model.playclicked) this.hand.removeAllEventListeners("tick");
  ...
  function drag(e) {
    ...
    this.layout.whiteHand.x =
      array[0].x +
      (array[array.length - 1].x - array[0].x) * Model.positionFactor;
  ...
  function tick() {
    ...
    this.layout.timeText.text =
      addCommas((Model.timer * 10).toFixed(1)) + " ms";
  }
  ...
  Class.prototype = {
    ...
    _drag: function (e) {
      ...
      Model.hand.xCheckPoints.forEach(function (h, i) {
        if (
          p.x > Model.hand.xCheckPoints[i] &&
          p.x < Model.hand.xCheckPoints[i + 1]
        ) {
...
var Wave = (function () {
  ...
  Class.prototype = {
    ...
    _tick: function () {
      ...
      scope.draw(
        Model.wavePoints["wave_" + Model.currentId],
        (Model.time * 100) % (Model.period * 100),
      );
    ...
    draw: function (array, i) {
      if (array && i) {
        ...
        scope.wave.graphics
          ...
          .setStrokeStyle(Model.canvasData.lineWidth)
          .beginStroke(Model.canvasData.color)
          .moveTo(Model.points[0].x, Model.points[0].y);
