...
var Lab = (function () {
  ...
  Activity.prototype.startLab = function () {
    ...
    for (var i = 0; i <= 2.75; i += this._pointGapX) {
      this._points.push(new Vector(i, 0));
    }
    console.log("this._points.length---->" + this._points.length);
    ...
    this.layout.timeStepper.addEventListener("valueChange", function (e) {
      ...
      for (k = 0; k < m; k++) {
        ...
        if (
          scope._currTimeVal < scope._prevTimeVal ||
          (scope._currTimeVal == scope._prevTimeVal && scope._inc == false)
        ) {
          console.log("innnn");
          scope._inc = false;
          if (scope._curTime > 0) {
            ...
            if (scope._waveEnd >= scope._points.length - 1) {
              scope._waveEnd2 += 6;
              if (scope._flag3) scope._waveStart2 += 6;
              //console.log("scope._points.length==="+scope._points.length);

              if (scope._waveStart2 > scope._points.length)
                scope._waveStart2 = scope._points.length - 1;
  ...
  function enterFrame(e, val) {
    ...
    for (var ll = 0; ll < _ll; ll++) {
      ...
      if (pAtStart2) {
        this._thetaAtStart2 =
          2 * Math.PI * this._F2 * (pAtStart2.x + this._phase2);
      }

      //console.log("this._thetaAtStart2---->"+this._thetaAtStart2);
      if (this._thetaAtStart2 >= Math.PI) {
        this._waveStart2 -= 3;
        this._flag3 = true;
      } else {
        this._flag3 = false;
      ...
      for (; i2 > this._waveEnd2; i2--) {
        ...
        if (p) {
          ...
          p.y +=
            this._A2 * Math.sin(2 * Math.PI * this._F2 * (p.x + this._phase2));
          //if(i2==(this._waveEnd2+16)&&(this._waveEnd2<942))  p.y+=0.005;
      ...
      for (i3 = this._waveStart2; i3 < this._points.length; i3++) {
        p = this._points[i3];
        if (p) {
          //console.log("before here p.y----"+p.y);
          //console.log("i3---"+i3);
    ...
    this.layout.waveshape.graphics
      ...
      .setStrokeStyle(3)
      .beginStroke("#000")
      .moveTo(this._points[0].x * this.unit, this._points[0].y * this.unit);
  ...
  Activity.prototype.reset = function () {
    ...
    this._waveEnd = 4;
    this._inc = true;
    this._phase = -1 * this._pointGapX * 3;
