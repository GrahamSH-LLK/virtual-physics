var Lab = (function () {
  ...
  Activity.prototype.startLab = function () {
    ...
    this.layout.stepper
      .enable()
      .addEventListener("valueChange", this._stepperClicked);
  ...
  function molPosition() {
    changeInVolume.call(this, {
      currentTarget: { value: this._defaultVolume },
    });
    var rows = 8,
      cols = 8;
    ...
    while (this._defaultPositions.length < this._totalMolecules) {
      (r = getRandomNum(0, rows - 1)), (c = getRandomNum(0, cols - 1));
      var comb = c + "," + r;
      ...
      if (combinations.indexOf(comb) == -1) {
        ...
        this._defaultPositions.push({
          x:
            Model.boundary.x +
            r * w +
            getRandomNum(this._moleculeRadius, w - this._moleculeRadius),
          y:
            Model.boundary.y +
            c * h +
            getRandomNum(this._moleculeRadius, h - this._moleculeRadius),
        });
      }
    }
  }
  ...
  function setTemprature(e) {
    ...
    if (newTemp != this._temperature) {
      ...
      this._m_gIdealAvgSpeed = Math.sqrt(
        ((8 / Math.PI) * this._R * this._temperature) / 0.018,
      );
      ...
      this.molecules.forEach(function (molecule) {
        ...
        molecule.m_gSpeed =
          Math.sqrt(((8 / Math.PI) * this._R * this._temperature) / 0.018) *
          fudgeFactor;
  ...
  function calcPressure() {
    ...
    this._Pressure =
      ((this._totalMolecules / 6.022e23) *
        0.018 *
        Math.pow(speed * Math.sqrt(3 / 2.54), 2)) /
      (3 * volume);
  ...
  Activity.prototype.reset = function () {
    ...
    this.molecules.forEach(function (m, i) {
      m.reset(
        this._defaultPositions[i],
        Model.pointToPixel(this._defaultVelocity),
      );
    }, this);
    this._temperature = 0;
...
this.layout.pressureText.text = this._Pressure.toFixed(0)
  ...
  function changeInVolume(e) {
    ...
    var newBond =
      3 + 2 * ((this._moleculeRadius * (scaleFactorX + scaleFactorY)) / 2);
    var newHeight =
      ((Model.boundary.defaultHeight - newBond) /
        ((3.2 - 0.1) * Math.pow(10, -22))) *
        (this._volume - 0.1 * Math.pow(10, -22)) +
      newBond;
    this.layout.piston.y = Model.boundary.defaultHeight - newHeight;
    Model.boundary.y =
      Model.boundary.defaultY + (Model.boundary.defaultHeight - newHeight);
  ...
  function createMolecules() {
    ...
    for (var i = 0; i < this._totalMolecules; i++) {
      ...
      m = new GasPart(
        mc,
        (this._moleculeRadius * (mc.scaleX + mc.scaleY)) / 2,
        this._moleculeMass,
        Model.step,
      );
