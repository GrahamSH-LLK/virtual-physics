var Lab = (function () {
  ...
  Activity.prototype.startLab = function () {
    var scope = this,
      ring2 = this.layout.ring2;

    this._ringHitArea = [
      new createjs.Rectangle(ring2.x - 5, ring2.y - ring2.regY - 5, 20, 23),
      new createjs.Rectangle(ring2.x + 6, ring2.y - ring2.regY + 72, 12, 23),
    ];

    // var shape = new createjs.Shape();
    // shape.graphics.beginFill("red").drawRect(this._ringHitArea[0].x, this._ringHitArea[0].y, this._ringHitArea[0].width, this._ringHitArea[0].height);
    // shape.graphics.drawRect(this._ringHitArea[1].x, this._ringHitArea[1].y, this._ringHitArea[1].width, this._ringHitArea[1].height);
    // this.addChild(shape);
    ...
    this.layout.mc_animation.addEventListener("end", function () {
      scope.layout.mc_animation.visible = false;
      scope.layout.mc_stick.visible = true;
      scope.layout.ring2.visible = true;
      scope.layout.ring1.visible = true;
    });
    ...
    this.allBalls = ["redBall", "blueBall", "greendBall"];
  ...
  Activity.prototype._ringRing = function () {
    this.layout.mc_animation.gotoAndPlay(0);
    this.layout.mc_animation.loop = false;
    this.layout.mc_animation.visible = true;
    this.layout.mc_stick.visible = false;
    this.layout.ring1.visible = false;
    this.layout.ring2.visible = false;
  };

  Activity.prototype._checkRingHit = function (ball) {
    var scope = this;
    if (this._ringHitArea[0].contains(ball.x, ball.y)) {
      return true;
    }
    if (this._ringHitArea[1].contains(ball.x, ball.y)) {
      return true;
    }
    return false;
  };
  ...
  Activity.prototype._play = function () {
    ...
    if (!this.started) {
      ...
      this.allBalls.forEach(function (ball, i) {
        ...
        ball.hand = this.leftHand;
        ball.Vx = -this.layout.stepperX.value;
  ...
  Activity.prototype.reset = function (e) {
    ...
    this.layout.mc_animation.visible = false;
    this.layout.mc_stick.visible = true;
    this.layout.ring2.visible = true;
    this.layout.ring1.visible = true;
  };
...
var HandClass = (function () {
  ...
  Class.prototype.catchCondition = function (ball) {
    if (ball.y >= this.y - 5 && ball.y <= this.y) {
      if (
        ball.x >= this.x - this.leftThreshold &&
        ball.x <= this.x + this.rightThreshold
      ) {
        return true;
      }
    }
  ...
  Class.prototype.catchAndThrow = function (ball) {
    ...
    createjs.Tween.get(this.displayObject, { override: true })
      .to({ x: this.defaults.x }, 0)
      .to({ x: this.defaults.x + this.offset * this.direction }, d);
    createjs.Tween.get(this.displayObject)
      .to(
        { y: this.defaults.y + this.offset / 2 },
        d / 2,
        createjs.Ease.circOut,
      )
      .to({ y: this.defaults.y }, d / 2, createjs.Ease.circIn)
      .call(function () {
        scope.catched = false;
        scope._updateBall();
        ...
        if (scope.ball) scope.ball.inHand = false;
        scope.ball = null;
        e.data = {
          ball: ball,
        };
...
var BallClass = (function () {
  ...
  Class.prototype.propogate = function () {
    ...
    this.Vy =
      this.Vy +
      /*this.Vy * this.timeStep + this.timeStep **/ this.timeStep *
        this._gravity;
