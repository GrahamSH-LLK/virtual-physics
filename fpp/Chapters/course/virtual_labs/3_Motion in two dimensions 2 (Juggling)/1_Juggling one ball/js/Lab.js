var Lab = (function () {
  ...
  function moveBall() {
    if (this.currentFrameCounter < 9) {
      ...
      (this.blueBall.xPos > 405) & (this.blueBall.xPos < 415)
        ? (flagEnd = 0)
        : (this.blueBall.xPos > 568) & (this.blueBall.xPos < 584)
          ? (flagEnd = 1)
          : null;

      if (this.currentFrameCounter == 0) makeImageAnimate(flagEnd, true, this);
      else makeImageAnimate(flagEnd, false, this);
    }
    ...
    if (this.blueBall.firstTripR) {
      ...
      this.sy =
        this.blueBall.yVelocity * this.elapsedTimeInSec +
        0.5 * -9.8 * (this.elapsedTimeInSec * this.elapsedTimeInSec);
    }

    //set the y velocity text
    this.blueBall.yVelocityDisp =
      (this.blueBall.yVelocity + -9.8 * this.elapsedTimeInSec).toFixed(1) * 1;
    ...
    var angle =
      (180 / Math.PI) *
      Math.abs(
        Math.atan((this.blueBall.yVelocityDisp * 1) / this.blueBall.xVelocity),
      );

    if (
      this.blueBall.xPos >= 568 &&
      this.blueBall.xPos <= 584 &&
      this.blueBall.yVelocityDisp < -this.blueBall.yVelocity + 0.4 &&
      this.blueBall.yVelocityDisp > -this.blueBall.yVelocity - 0.4 &&
      ...
      this.blueBall.xPos >= 405 &&
      this.blueBall.xPos <= 415 &&
      this.blueBall.yPos >= 658 &&
      ...
      if (this.blueBall.xVelocity <= 0.1) {
        ...
        this.blueBall.xVelocity < 0.7 &&
        this.blueBall.yVelocity < 0.7
      ) {
        this.blueBall.xVelocity = 0;
        this.blueBall.yVelocity = 0;
        makeImageAnimate(0, true, this);
      } else if (
        this.blueBall.yVelocity == 0.0 &&
        this.blueBall.xVelocity > 0.1
      ) {
        this.blueBall.xVelocity = 0;
        this.blueBall.yVelocity = 0;
        makeImageAnimate(0, true, this);
      ...
      this.layout.blueBall.x + 8 >= 412.5 - 8 &&
      this.layout.blueBall.x + 8 <= 422.5 + 8
    ) {
  ...
  function makeImageAnimate(flagEnd, makeZero, _thisRef) {
    ...
    if (flagEnd == 0) {
      if (_thisRef.leftHandCtr == _thisRef.leftHandMovementArray.length) {
        ...
        _thisRef.layout.leftHand.x =
          _thisRef.leftHandMovementArray[_thisRef.leftHandCtr][0];
        _thisRef.layout.leftHand.y =
          _thisRef.leftHandMovementArray[_thisRef.leftHandCtr][1];
    ...
    if (flagEnd == 1) {
      if (_thisRef.rightHandCtr == _thisRef.rightHandMovementArray.length) {
        ...
        _thisRef.layout.rightHand.x =
          _thisRef.rightHandMovementArray[_thisRef.rightHandCtr][0];
        _thisRef.layout.rightHand.y =
          _thisRef.rightHandMovementArray[_thisRef.rightHandCtr][1];
