var Lab = (function () {
  ...
  Activity.prototype.startLab = function () {
    ...
    this.layout.stepper1.addEventListener("valueChange", function (e) {
      scope.layout.speed1.text = e.currentTarget.value.toFixed(1);

      scope.cars.redCar.maxVelocity[0] = e.currentTarget.value;
    });

    this.layout.stepper2.addEventListener("valueChange", function (e) {
      scope.layout.speed2.text = e.currentTarget.value.toFixed(1);

      scope.cars.redCar.maxVelocity[1] = e.currentTarget.value;
    });

    this.layout.stepper3.addEventListener("valueChange", function (e) {
      scope.layout.speed3.text = e.currentTarget.value.toFixed(1);

      scope.cars.redCar.maxVelocity[2] = e.currentTarget.value;
    });
  ...
  // function keydown(e){
  // if(e.keyCode==38){
  // this.layout.stepper1.handleEvent({type:"mousedown",currentTarget:this.layout.stepper1._inc})
  // }
  // else if(e.keyCode==40){
  // this.layout.stepper1.handleEvent({type:"mousedown",currentTarget:this.layout.stepper1._dec})
  // }
  // }

  // function keyup(e){
  // this.layout.stepper1.handleEvent({});
  // }
  ...
  Activity.prototype.reset = function () {
    ...
    scope.layout.stepper1.enable();
    scope.layout.stepper2.enable();
    scope.layout.stepper3.enable();
...
var control = function () {
  ...
  function updateState() {
    for (var i = 0; i < 2; i++) {
      ...
      if (this.position > 2 && this.displacementX < 379) {
        if (this.name == "redCar") {
          lab._pause();
      ...
      if (this.running) {
        ...
        if (this.movingStraight == true) {
          if (this.position < 3) {
            if (this.velocityX < this.maxVelocity[this.position]) {
              this.velocityX =
                this.velocityX + this.accelerationX * timeStep * 0.11;
              if (
                (this.velocityX >= 0 ||
                  this.displacementX <
                    this.track[this.position].center.x - 1) &&
                this.position == 1 &&
                !this._checkSpeed
              ) {
                this.velocityX = this.maxVelocity[this.position];
              }
            } else if (this.velocityX > this.maxVelocity[this.position]) {
              this.velocityX =
                this.velocityX + this.accelerationX * timeStep * 0.11;
              if (
                (this.velocityX <= 0 ||
            ...
            // if(this.velocityX <= this.maxVelocity[this.position])
            // {
            // this.velocityX =this.velocityX +(this.accelerationX *timeStep * 0.11);
            // }
            // else if(this.velocityX >= this.maxVelocity[this.position]){
            // this.velocityX =this.velocityX -(this.accelerationX *timeStep * 0.11);
            // }
          ...
          if (this.name == "redCar" && this.flag == true) {
            if (lab.layout.redCar.x > 890) {
              if (this.position == 1) {
                lab.layout.redCrash.rotation = lab.layout.redCar.rotation + 80;
              } else {
                lab.layout.redCrash.rotation = lab.layout.redCar.rotation - 80;
              }
            ...
            } else if (lab.layout.redCar.x < 120) {
              lab.layout.redCrash.rotation = lab.layout.redCar.rotation - 80;
              lab.layout.redCrash.x = lab.layout.redCar.x;
              lab.layout.redCrash.y = lab.layout.redCar.y;
              lab.layout.redCrash.visible = true;
          ...
          this.centripetalAcc = Math.abs(
            (this.velocity.toFixed(1) * this.velocity.toFixed(1)) /
              this.track[this.position].radius,
          );
          if (this.name == "redCar") {
  ...
  function startConditions() {
    // if(this.maxVelocity[this.position] != 0){
    if (this.position == 0) {
      this.accelerationX =
        (0.5 *
          (this.maxVelocity[this.position] * this.maxVelocity[this.position])) /
        26.2;
    } else if (this.position == 1) {
      this.vdiff =
        this.maxVelocity[this.position] * this.maxVelocity[this.position] -
        this.maxVelocity[this.position - 1] *
          this.maxVelocity[this.position - 1];
      this.accelerationX = -((0.5 * this.vdiff) / 39.465);
    ...
    // else{
    // if(this.position == 1){
    // this.vdiff=Math.abs(this.maxVelocity[this.position-1] * this.maxVelocity[this.position-1]);
    // this.accelerationX = -(0.5 * (this.vdiff)/39.465);
    // }
    // else{
    // this.vdiff= -(this.maxVelocity[this.position-1] * this.maxVelocity[this.position-1]);
    // this.accelerationX = (0.5 * (this.vdiff)/39.465);
    // }
    // }
