var Lab = (function () {
  ...
  Activity.prototype.startLab = function () {
    ...
    this.layout.helicopter.snapToPixel = false;
    this.reset();
  };
  ...
  function calculator() {
    ...
    if (
      this.displacementH > this.LEFT_BORDER &&
      this.displacementH < this.RIGHT_BORDER
    ) {
  ...
  function checkLanding() {
    if (
      (this.displacementH <= 305 && this.displacementH >= 73) ||
      (this.displacementH >= 950 && this.displacementH <= 1170)
    ) {
      if (
        this.velocityH <= 2.6 &&
        this.velocityH >= -2.6 &&
  ...
  function checkDir() {
    if (this.velocityH < 0) {
      this.layout.helicopter.scaleX = -1.2;
      this.layout.helicopterBreak.scaleX = -1.2;
  ...
  function play() {
    this.layout.helicopter.children[0].play();
  ...
  function pause() {
    this.layout.helicopter.children[0].stop();
    createjs.Ticker.removeEventListener("tick", this._calculator);
  ...
  function keydown(e) {
    if (e.keyCode == 39) {
      this.layout.stepperH.handleEvent({
        type: "mousedown",
        currentTarget: this.layout.stepperH._inc,
      });
    } else if (e.keyCode == 37) {
      this.layout.stepperH.handleEvent({
        type: "mousedown",
        currentTarget: this.layout.stepperH._dec,
      });
    } else if (e.keyCode == 38) {
      this.layout.stepperV.handleEvent({
        type: "mousedown",
        currentTarget: this.layout.stepperV._inc,
      });
    } else if (e.keyCode == 40) {
      this.layout.stepperV.handleEvent({
        type: "mousedown",
        currentTarget: this.layout.stepperV._dec,
      });
    }
  }
  ...
  Activity.prototype.reset = function () {
    ...
    this.layout.helicopter.children[0].play();
    this.layout.helicopter.setTransform(
      190,
      420,
      1.2,
      1.2,
