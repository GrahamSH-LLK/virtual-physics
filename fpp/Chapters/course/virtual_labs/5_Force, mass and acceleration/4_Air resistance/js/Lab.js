var Lab = (function () {
  ...
  Activity.prototype.startLab = function () {
    ...
    this.layout.stepper.enable().addEventListener("valueChange", function (e) {
      ...
      if (
        e.currentTarget._direction == 1 &&
        e.currentTarget.value < scope.MAX_THRUST
      ) {
        scope.accleration = scope.accleration + scope.ACCELERATION_STEP;
      } else if (
        e.currentTarget._direction == -1 &&
        e.currentTarget.value > scope.MIN_THRUST
      ) {
        scope.accleration = scope.accleration - scope.ACCELERATION_STEP;
      }

      if (scope.flag == 0) {
        if (scope.accleration.toFixed(2) == 0.0) {
  ...
  function calculator() {
    ...
    this.displacement =
      this.displacement + this.velocity * this.CONVERT_PIXEL * this.TIMESTEP;
    if (this.accleration.toFixed(2) == 0.0) {
      this.layout.h_accleration.text = "0.00";
    } else {
      this.layout.h_accleration.text = this.accleration.toFixed(2);
    }
    if (this.velocity.toFixed(2) == 0.0) {
    ...
    if (
      this.displacement > this.LEFT_BORDER &&
      this.displacement < this.RIGHT_BORDER
    ) {
