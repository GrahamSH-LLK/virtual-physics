var Lab = (function () {
  ...
  Activity.prototype.startLab = function () {
    ...
    //	this.layout.playButton.addEventListener("click", this._goClicked);
    ...
    var a = (this.a = new Hit(
      this.layout.hitbutton,
      this.layout.hitrod,
      this.layout.ball,
      ballData.draggableMin,
      ballData.draggableMax,
      ballData.draggableMin1,
      ballData.draggableMax1,
      ballData.minStep,
      ballData.maxStep,
    ));
    this.a.enable();
    ...
    //	scope.layout.playButton.enable();
  ...
  function drag() {
    ...
    scope.velocity = scope.a.reverse
      ? (
          Math.round(
            -(scope.layout.hitbutton.x - Model.Ball.draggableMin1) / 2,
          ) * 0.1
        ).toFixed(2)
      : (
          Math.round((Model.Ball.draggableMax - scope.layout.hitbutton.x) / 2) *
          0.1
        ).toFixed(2);
    scope.layout.speedText.text = scope.a.reverse
      ? (
          Math.round(
            -(scope.layout.hitbutton.x - Model.Ball.draggableMin1) / 2,
          ) * 0.1
  ...
  function updateScore() {
    Model.score =
      Model.currentSlot != -1
        ? Model.score + Model.pPointsValue[Model.currentSlot]
        : Model.score - 10;
  ...
  function goClicked(e) {
    if (Model.shots == Model.maxShots) {
      ...
      scope.a.fire(lab.layout.speedText.text * 1);
    }
  }
...
var Model = {
  ...
  time: 0,
  acceleration: 2,
  pHolesRadiusOuter: 40,
  pHolesRadiusInner: 21,
  ...
  Class.prototype = {
    ...
    _tick: function () {
      ...
      if (Model.time < 2.01) {
        ...
        var outerDistance =
          (Model.ballRadius + Model.pHolesRadiusOuter).toFixed(0) * 1;

        // console.log(innerDistance, outerDistance)
        for (; i < pPointsValue.length; i++) {
          var currentDistance =
            Model.getDistance(pPointsValue[i], _p).toFixed(0) * 1;
          var nextDistance =
            Model.getDistance(pPointsValue[i], _nextP).toFixed(0) * 1;
          if (
            currentDistance >= outerDistance &&
            nextDistance <= outerDistance
          ) {
            Model.vectorS = new Vector(0, 0);
            Model.vectorM = new Vector(Model.velocityX, Model.velocityY);
            var current = Model.pHolesCenter[i];
            var n = Vector.subtract(
              new Vector(current.x, current.y, 0),
              new Vector(_p.x, _p.y, 0),
            ).divide(currentDistance);
            var p =
              (2 * (Model.vectorM.dot(n) - Model.vectorS.dot(n))) / (129 + 129);
          ...
          } else if (currentDistance <= innerDistance) {
            //	console.log("innner" +i);
            //	console.log("curr"+currentDistance+"out"+outerDistance)
            Model.currentSlot = i;
            this.ball.scaleX = 1;
            this.ball.scaleY = 1;
        ...
        if (
          this.ball.y >= Model.stopperPoints[0].y &&
          this.ball.y <= Model.stopperPoints[2].y
        ) {
          var x = Model.getX(
            Model.stopperPoints[0],
            Model.stopperPoints[1],
            this.ball.y,
          );
          var stopperPoint = {
            x: x,
          ...
          if (
            Model.ballRadius >= _distance ||
            Model.ballRadius + 10 >= _distance
          ) {
  ...
  Class.prototype = {
    ...
    _drag: function (e) {
      ...
      if (e.stageX <= (this.ball.x - this.ball.regX) * foregroundStage.scaleX) {
        ...
        this.rod.scaleX =
          this.scaleFactor * this.hit.x > 1.421
            ? 1.421
            : this.scaleFactor * this.hit.x;
        this.hit.rotation = 0;
        this.rod.rotation = 0;
        this.rod.x = -3;
        ...
        this.rod.scaleX =
          (this.rod.x - this.hit.x) * this.scaleFactor > 1.421
            ? 1.421
            : (this.rod.x - this.hit.x) * this.scaleFactor;
      }
