var Lab = (function () {
  ...
  Activity.prototype.startLab = function () {
    ...
    this.layout.vSpeedStepper.addEventListener("valueChange", function (e) {
      console.log(e.currentTarget._direction);

      v1.set(0, e.currentTarget._direction * e.currentTarget._step * -1);
      if (
        scope.playerIndex == 0 &&
        scope.activePlayer.arrow.angle == scope.activePlayer.arrow._maxAngle
      ) {
        v1.set(0, e.currentTarget._direction * e.currentTarget._step * -2);
      } else if (
        scope.playerIndex == 1 &&
        scope.activePlayer.arrow.angle == scope.activePlayer.arrow._minAngle
      ) {
        v1.set(0, e.currentTarget._direction * e.currentTarget._step * 2);
      }
  ...
  Activity.prototype.reset = function () {
    var targetValue1 = Model.randomNumber(2, 7) * 20;
    var targetValue2 = Model.randomNumber(2, 7) * 20;
    if (Math.abs(targetValue1 - 90) == Math.abs(targetValue2 - 90)) {
    ...
    this.layout.tower_blast_1.cache.apply(
      this.layout.tower_blast_1,
      this.cacheData.tower_blast,
    );
    this.layout.tower_blast_1.removeAllEventListeners("tick");
    this.layout.tower_blast_2.gotoAndStop(0);
    this.layout.tower_blast_2.cache.apply(
      this.layout.tower_blast_2,
      this.cacheData.tower_blast,
    );
    ...
    this.player1 /* .data("angle", this.player1.data("defaultA")) */
      .reset();
    this.player2 /* .data("velocity", this.player2.data("defaultV")) */
      .data("point0", false)
      .data("point1", false);
    this.player2 /* .data("angle", this.player2.data("defaultA")) */
      .reset();
    this.player1.turn = 0;
    this.player2.turn = 0;
  ...
  function fireClicked(e) {
    ...
    if (this.activePlayer.auto) {
      ...
      this.activePlayer.fire(
        true,
        this.layout.hSpeedStepper.value,
        this.layout.vSpeedStepper.value,
      );
    }
  ...
  function setGameMode(e) {
    this.layout.compBtn
      .disable()
      .removeEventListener("click", this._setGameMode);
    this.layout.playerBtn
      .disable()
      .removeEventListener("click", this._setGameMode);
    this.layout.playerSelection.visible = false;
  ...
  function nextTurn() {
    ...
    if (this.activePlayer) {
      ...
      if (player) {
        ...
        createjs.Tween.get(this.layout.playAgainScreen)
          .wait(500)
          .to({ alpha: 1 }, 500)
          .call(function () {});
        return false;
    ...
    this.layout.hSpeedStepper.updateMinMax(
      this.activePlayer.data("minX"),
      this.activePlayer.data("maxX"),
      0.1,
      this.activePlayer.data("X"),
    );
    ...
    // this.layout.vSpeedStepper.value = this.activePlayer.velocity;
    // this.layout.hSpeedStepper.value = this.activePlayer.angle;
    ...
    this._onDataChange({ type: "dataChange" });
  ...
  function createPlayers() {
    ...
    this.vLine1 = new VectorLine(
      this.layout.arrow1,
      { x: this.layout.cannon1.x, y: this.layout.cannon1.y },
      { min: 15, max: 100 },
      { min: 270, max: 270 + 45 },
    ...
    this.player1 = new Player(
      this.layout.p1thumb,
      [this.layout.point2_1, this.layout.point2_2],
      this.vLine1,
      this.layout.canonBall1,
    );
    this.vLine1.unit = 4;
    this.vLine1.enable();
    this.vLine2 = new VectorLine(
      this.layout.arrow2,
      {
      ...
      { min: 15, max: 100 },
      { min: 270 - 45, max: 270 },
      this.layout.arrow2Line,
      { color: "#F22E1E" },
      0.5,
    );
    this.player2 = new Player(
      this.layout.p2thumb,
      [this.layout.point1_1, this.layout.point1_2],
      this.vLine2,
      this.layout.canonBall2,
    );
    this.vLine2.unit = 4;
    this.vLine2.enable();
    ...
    this.player1
      .data("velocity", this.player1.data("defaultV"))
      .data("angle", this.player1.data("defaultA"));
    this.player2
      .data("velocity", this.player2.data("defaultV"))
      .data("angle", this.player2.data("defaultA"));
    this.player1 /* .reset() */
      .addEventListener("dataChange", this._onDataChange);
    this.player2 /* .reset() */
      .addEventListener("dataChange", this._onDataChange);
  ...
  function onDataChange(e) {
    if (this.activePlayer.auto) {
      this.layout.vSpeedText.text = this.layout.hSpeedText.text = "---";
      this.layout.hSpeedStepper.disable();
      this.layout.vSpeedStepper.disable();

      this.layout["cannon" + (this.playerIndex + 1)].rotation =
        this.layout["cannon" + (this.playerIndex + 1)]._defAngle +
        this.activePlayer.angle;
      return false;
    }
    var scope = this,
      _p;
    if (this.activePlayer.arrow.y < -60 * this.activePlayer.arrow.unit) {
      _p = Model.doIntersect(
        { x: -1000, y: -60 * this.activePlayer.arrow.unit },
        { x: 1024, y: -60 * this.activePlayer.arrow.unit },
        { x: 0, y: 0 },
        scope.activePlayer.arrow._vector,
      );
      // if(this.playerIndex == 0){
      // _p = new Vector(Math.max(0, _p.x), _p.y);
      // } else {
      // _p = new Vector(_p.x, _p.y);
      // }
    } else if (this.activePlayer.arrow.y > -35 * this.activePlayer.arrow.unit) {
      var v = Vector.components(
        2000,
        scope.activePlayer.arrow.angle * Maths.TO_RADIANS,
      );
      _p = Model.doIntersect(
        { x: -200, y: -35 * this.activePlayer.arrow.unit },
        { x: 1024, y: -35 * this.activePlayer.arrow.unit },
        { x: 0, y: 0 },
        v,
      );
    }
    if (_p) {
      ...
      scope.activePlayer.arrow.magnitude =
        _p.magnitude() / scope.activePlayer.arrow.unit;
      scope.activePlayer.arrow.angle = 360 + _p.angle();
    }
    this.activePlayer.arrow.y = this.activePlayer.arrow.y.toFixed(0) * 1;
    this.activePlayer.arrow.x = this.activePlayer.arrow.x.toFixed(0) * 1;
    var v =
      Math.abs(
        this.activePlayer.arrow.y / this.activePlayer.arrow.unit,
      ).toFixed(1) * 1;
    var h =
      (this.activePlayer.arrow.x / this.activePlayer.arrow.unit).toFixed(1) * 1;
    if (e && e.type == "dataChange") {
      //
      this.layout.hSpeedStepper.value = this.layout.hSpeedStepper._min + 1;
      this.layout.vSpeedStepper.value = this.layout.vSpeedStepper._min + 1;
      //
      this.layout.hSpeedStepper.value = h;
      this.layout.vSpeedStepper.value = v;
    }
    if (e == true) {
      this.layout.hSpeedStepper.value = h;
    }
    this.layout.hSpeedText.text = h.toFixed(1);
    this.layout.vSpeedText.text = v.toFixed(1);
    this.activePlayer.data("X", this.layout.hSpeedStepper.value);
    // if(this.playerIndex == 1){
    // this.layout.hSpeedText.text = "-" + h.toFixed(1);
    // }
    this.layout["cannon" + (this.playerIndex + 1)].rotation =
  ...
  function checkFlagHit(_i) {
    ...
    var tolerance = Model.pointToPixel(3);
    var res = {};
  ...
  function hit(e) {
    ...
    switch (e.data.hit) {
      case 7:
        var res = /* false// */ checkFlagHit.call(this, e.data.point);
        // console.log(res);
      ...
      case 11:
        if (!this.activePlayer.data("point0")) {
          ...
          mc =
            this.layout["tower_blast_" + (this.playerIndex == 0 ? "2" : "1")];
          mc.uncache();
          mc.addEventListener("end", function () {
            mc.removeAllEventListeners("end");
...
Lab.data = {
  player1: {
    direction: 1,
    defaultV: 38.08613046095689,
    // defaultV:53.613098520817545,
    defaultA: 293.2240786906963,
    // defaultA:291.66224683040446,
    startX: 199,
    soldierEndX: 775,
    minX: 0,
    maxX: 60,
  ...
  Class.prototype = {
    ...
    setTargets: function (array) {
      ...
      this.pointers.forEach(function (p, i) {
        x = p.x =
          this.data("startX") +
          Model.pointToPixel(array[i]) * this.data("direction");
      }, this);
      return x;
    },
    _onDrag: function (e) {
    ...
    nextV: function () {
      ...
      spd = Math.sqrt(
        Math.abs((9.8 * hDist) / Math.sin(2 * angle * Maths.TO_RADIANS)),
      );
      if (i != 1) {
        if (rand2 < 4) {
          spd -= 0.1;
    ...
    fire: function (bool, Vx, Vy) {
      if (arguments.length > 0) {
        this.data("velocity", this.arrow.magnitude);
        this.data("angle", this.arrow.angle);
        var p = this.arrow._baseVector.add(
          Vector.components(22, this.arrow.angle * Maths.TO_RADIANS),
        );
        // console.log(p)
        this.ball.x = p.x;
        this.ball.y = /* 545// */ p.y;
        var v = Vector.components(
          this.arrow.magnitude,
          this.arrow.angle * Maths.TO_RADIANS,
        );
        // console.log(this.data("direction"))
        this.data("Vx", typeof Vx == "undefined" ? v.x : Vx);
        this.data("Vy", typeof Vy == "undefined" ? -v.y : Vy);
...
Model.doIntersect = function (p1, q1, p2, q2 /* , *i_x, *i_y */) {
  ...
  var s, t;
  s =
    (-s1_y * (p1.x - p2.x) + s1_x * (p1.y - p2.y)) /
    (-s2_x * s1_y + s1_x * s2_y);
  t =
    (s2_x * (p1.y - p2.y) - s2_y * (p1.x - p2.x)) /
    (-s2_x * s1_y + s1_x * s2_y);
