var Lab = (function () {
  ...
  Activity.prototype.startLab = function () {
    ...
    this.layout.angleStepper.addEventListener("valueChange", function (e) {
      ...
      // scope.activePlayer.data("angle", scope.activePlayer.arrow.angle);
      scope.layout["cannon" + (scope.playerIndex + 1)].rotation =
        scope.layout["cannon" + (scope.playerIndex + 1)]._defAngle +
        scope.activePlayer.angle;
    });
    this._extraMCs = [];
  ...
  function onDataChange(e) {
    ...
    this.layout.angleStepper.value =
      (360 - this.activePlayer.angle).toFixed(0) * 1;
  ...
  function hit(e) {
    ...
    var v =
      Vector.components(
        this.layout.speedStepper.value,
        this.layout.angleStepper.value * Math.TO_RADIANS,
      ).y.toFixed(1) * 1;
    ...
    fire: function (bool) {
      if (arguments.length > 0) {
        ...
        var p = this.arrow._baseVector.add(
          Vector.components(39, this.arrow.angle * Maths.TO_RADIANS),
        );
        this.ball.x = p.x;
        this.ball.y = p.y;
