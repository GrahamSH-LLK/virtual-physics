var Lab = (function () {
  ...
  Activity.prototype.startLab = function () {
    ...
    shape.graphics
      .clear()
      .beginFill("#fff")
      .rect.apply(shape.graphics, this._enemyMaskRect)
      .endFill();
  ...
  Activity.prototype.reset = function () {
    ...
    this.layout.towerman.cache.apply(
      this.layout.towerman,
      this._cacheData.towerman,
    );
    ...
    this.layout.windowAnim.cache.apply(
      this.layout.windowAnim,
      this._cacheData.windowAnim,
    );
  ...
  function cacheSoldiers() {
    this.layout.enemyrun.cache.apply(
      this.layout.enemyrun,
      this._cacheData.enemyrun,
    );
  }
  ...
  function createPlayers() {
    ...
    var a = new Slider(
      ...
      { direction: "left", lineWidth: BrowserDetect.any() ? 4 : 2 },
      "y",
    );
    a.reverse = true;
  ...
  function checkBallHeight() {
    ...
    if (
      Math.abs(this.cannon.ballHeight - Model.targetHeight) <=
      0.7 /*  && Math.abs(this.cannon.data("Vy")) < 0.2 */
    ) {
    ...
    this.layout.txtVy.text = Math.max(
      -1 * this.layout.stepper.value,
      this.cannon.data("Vy"),
    ).toFixed(1);
    if (this.cannon.data("Vy") >= -1 * this.layout.stepper.value) {
      ...
      this.layout.timeText.text =
        Math.abs((2 * this.layout.stepper.value) / Model.g).toFixed(2) + " s";
    }
    if (caught) {
      this.layout.timeText.text =
        Math.abs(this.layout.stepper.value / Model.g).toFixed(2) + " s";
      // console.log(this.layout.timeText.text)
      this.layout.txtVy.text =
        this.layout.txtAy.text =
        this.layout.txtAx.text =
