  ...
  function nextTurn() {
    ...
    if (count < 3) {
      if (!this.hasOwnProperty("playerIndex")) {
        ...
        if (!this.activePlayer.auto) {
          this.activePlayer =
            this["player_" + (this.activePlayer.playerIndex + 2)];
          this.activePlayer.enable();
          this.playerIndex = this.activePlayer.playerIndex;
        } else {
  ...
  function enableDisableUser(action) {
    ...
    Model.players.user.forEach(function (p, i) {
      if (action) {
        ...
        if (
          Model.helicopter.hit[p.playerIndex / 2] == false &&
          Model.submarine.hit[p.playerIndex / 2] == false
        )
          p.enable();
      } else {
        p.disable();
  ...
  Class.prototype = {
    ...
    nextAngle: function () {
      ...
      this.angle = this.cAngle =
        Model.submarine.Angles[(this.playerIndex - 1) / 2] -
