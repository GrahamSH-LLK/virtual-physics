var Lab = (function () {
  ...
  Activity.prototype.startLab = function () {
    ...
    this.player_1.addEventListener(
      "enableDisableUser",
      this._enableDisableUser,
    );
    this.player_2.addEventListener(
      "enableDisableUser",
      this._enableDisableUser,
    );
    this.player_3.addEventListener(
      "enableDisableUser",
      this._enableDisableUser,
    );
    this.player_4.addEventListener(
      "enableDisableUser",
      this._enableDisableUser,
    );
    this.player_5.addEventListener(
      "enableDisableUser",
      this._enableDisableUser,
    );
    this.player_6.addEventListener(
      "enableDisableUser",
      this._enableDisableUser,
    );
  ...
  function updateVelocity(e) {
    ...
    if (!e.status) {
      ...
      this.layout[
        "velocity" + (Math.floor(this.playerIndex / 2) * 1 + 1)
      ].text = velocity;
    } else if (e.status) {
      if (this.activePlayer.auto) {
        velocity = (3).toFixed(1);
  ...
  function createPlayers() {
    ...
    var a1 = new VectorLine(
      this.layout.arrowMiddle,
      {
        x: 427.9,
        y: 113.1,
      },
    ...
    var a2 = new VectorLine(
      this.layout.arrowRight,
      {
        x: 910,
        y: 68.65,
      },
...
var Player = (function () {
  ...
  Class.prototype = {
    ...
    nextAngle: function () {
      ...
      if (index == 0) variance = variance * 2;
      this.angle = this.cAngle =
        Model.submarine.Angles[(this.playerIndex - 1) / 2][index] +
        variance * 6;
    },
      ...
      if (angle < -142.3) {
        ...
        if (!this.auto) {
          var angle =
            90 -
            Math.asin(
              Math.sin((90 - angle) * Maths.TO_RADIANS) /
                Model.refractiveIndex[index],
            ) *
              Maths.TO_DEGREES;
          ...
          var angle =
            -90 +
            Math.asin(
              Math.sin((angle + 90) * Maths.TO_RADIANS) *
                Model.refractiveIndex[index],
            ) *
              Maths.TO_DEGREES;
    ...
    _displayCross: function () {
      if (!this.auto) {
        ...
        this.cross.graphics
          .setStrokeStyle(this.canvasData.lineWidth)
          .beginStroke(this.canvasData.color)
          .moveTo(cross.x - cross.width / 2, cross.y - cross.height / 2)
          .lineTo(cross.x + cross.width / 2, cross.y + cross.height / 2)
          .endStroke();
        this.cross.graphics
          .setStrokeStyle(this.canvasData.lineWidth)
          .beginStroke(this.canvasData.color)
    ...
    _tick: function () {
      ...
      if (this.touchedBoundry == false) {
        if (this.auto) {
          step =
            Model.step / Model.refractiveIndex[this.id.split("_")[1] * 1 - 1];
        } else {
          step = Model.step;
        }
      } else {
      ...
      if (
        curr.x > this.boundry.xMin &&
        curr.x < this.boundry.xMax &&
        curr.y > this.boundry.yMin &&
        curr.y < this.boundry.yMax
      ) {
        if (!this.auto) {
