var Lab = (function () {
  ...
  Activity.prototype.reset = function () {
    ...
    for (var i = 1; i <= 4; i++) this.layout["instruction" + i].visible = true;
  ...
  function createPlayers() {
    ...
    this.player2 = new Player(
      this.layout.player_2,
      Model.helicopter.options,
      null,
    );
    ...
    this.layout.stepper.addEventListener("valueChange", function (e) {
      ...
      a.angle =
        scope.layout.stepper.value % 360 > 0
          ? scope.layout.stepper.value % 360
          : -1 * (scope.layout.stepper.value % 360);
  ...
  function checkWinner(e) {
    ...
    var helAnglePoint,
      subAnglePoint,
      ended = false;
    helAnglePoint = Model.getPoint(
      Model.pointOnBoundry,
      -(90 - Model.currentHelAngle / 2) * Maths.TO_RADIANS,
      115,
    );
    subAnglePoint = Model.getPoint(
      Model.pointOnBoundry,
      (90 + Model.currentSubAngle / 2) * Maths.TO_RADIANS,
      100,
    );

    if (Model.currentHit >= 0 && this.activePlayer.auto == false) {
      lab.layout["helAngle" + (Model.currentHit + 1)].text =
        Model.currentHelAngle.toFixed(1) + "\u00B0";
      lab.layout["subAngle" + (Model.currentHit + 1)].text =
        Model.currentSubAngle.toFixed(1) + "\u00B0";
      lab.layout["helAngle" + (Model.currentHit + 1)].x = helAnglePoint.x;
      lab.layout["helAngle" + (Model.currentHit + 1)].y = helAnglePoint.y;
      lab.layout["subAngle" + (Model.currentHit + 1)].x = subAnglePoint.x;
      lab.layout["subAngle" + (Model.currentHit + 1)].y = subAnglePoint.y;
    ...
    if (Model.currentHit >= 0 && this.activePlayer.auto == true) {
      lab.layout["helAngle" + (Model.prevsub + 1)].text =
        Model.currentHelAngle.toFixed(1) + "\u00B0";
      lab.layout["subAngle" + (Model.prevsub + 1)].text =
        Model.currentSubAngle.toFixed(1) + "\u00B0";
      lab.layout["helAngle" + (Model.prevsub + 1)].y = helAnglePoint.y;
      lab.layout["subAngle" + (Model.prevsub + 1)].y = subAnglePoint.y;
      lab.layout["helAngle" + (Model.prevsub + 1)].x = helAnglePoint.x;
      lab.layout["subAngle" + (Model.prevsub + 1)].x = subAnglePoint.x;
    ...
    if (!this.activePlayer.auto) {
      ...
      if (count == 3) {
        ...
        for (var j = 1; j <= 2; j++) this.layout["correct" + j].visible = true;
        //	console.log(text);
      }
    } else {
      Model.helicopter.hit.forEach(function (h, i) {
  ...
  function displayCross() {
    ...
    //	cross.y = crossPoint.y + (crossPoint.height*3/4);
    //	cross.visible = true;
...
var Player = (function () {
  ...
  Class.prototype = {
    ...
    nextAngle: function () {
      if (Model.nextsub == 0) {
        if (Model.count < 1) {
          var variation = Model.randomNumber(0, 2);
          this.angle = this.cAngle =
            Model.submarine.Angles[Model.nextsub] - Model.variation[variation];
        } else {
          this.angle = this.cAngle = Model.submarine.Angles[Model.nextsub];
        }
      } else {
        this.angle = this.cAngle = Model.submarine.Angles[Model.nextsub];
      }
    ...
    refractedAngle: function (angle) {
      ...
      if (!this.auto) {
        if (90 - angle < 0)
          var angle =
            90 +
            Math.asin(Math.sin((angle - 90) * Maths.TO_RADIANS) / 1.33) *
              Maths.TO_DEGREES;
        else
          var angle =
            90 -
            Math.asin(Math.sin((90 - angle) * Maths.TO_RADIANS) / 1.33) *
              Maths.TO_DEGREES;
    ...
    _checkHit: function (point) {
      ...
      if (!this.auto) {
        Model.submarine.options.forEach(function (s, i) {
          if (!Model.submarine.hit[i]) {
            // if(s.y <= point.y && s.y + s.height >= point.y)
            // console.log(point , s.x <= point.x && s.x + s.width >= point.x && s.y + (s.height/3) <= point.y && s.y + s.height >= point.y)
            if (
              s.x <= point.x &&
              s.x + s.width >= point.x &&
              s.y + s.height / 3 <= point.y &&
              s.y + s.height >= point.y
            ) {
              status = true;
        ...
        Model.helicopter.options.forEach(function (s, i) {
          if (
            s.x <= point.x &&
            s.x + s.width >= point.x &&
            s.y + s.height / 4 <= point.y &&
            s.y + s.height / 2 >= point.y
          ) {
            status = true;
    ...
    drawAngles: function () {
      this.drawDotted(this.pointOnSurface.x, 3, this.pointOnSurface.x, 735, 15);
      this.drawArc();
    },
    drawArc: function () {
      ...
      var displayHelAngle =
          Model.helAngle >= 0 ? Model.helAngle - 90 : 90 + Model.helAngle,
        displaySubAngle =
          Model.subAngle >= 0 ? Model.subAngle - 90 : 90 + Model.subAngle;
      ...
      if (this.auto) {
        this.canvas.graphics
          .setStrokeStyle(2)
          .moveTo(firstHelPoint.x, firstHelPoint.y)
          .beginStroke("#000000");
        this.canvas.graphics.arc(
          point.x,
          point.y,
          90,
          -90 * Maths.TO_RADIANS,
          -(90 - displayHelAngle) * Maths.TO_RADIANS,
        );
        this.canvas.graphics.endStroke();
        this.canvas.graphics
          .setStrokeStyle(2)
          .moveTo(firstSubPoint.x, firstSubPoint.y)
          .beginStroke("#000000");
        this.canvas.graphics.arc(
          point.x,
          point.y,
          90,
          90 * Maths.TO_RADIANS,
          (displaySubAngle + 90) * Maths.TO_RADIANS,
        );
        this.canvas.graphics.endStroke();
      } else {
        this.canvas.graphics
        ...
        this.canvas.graphics.arc(
          ...
          90,
          90 * Maths.TO_RADIANS,
          Model.subAngle * Maths.TO_RADIANS,
        );
        this.canvas.graphics.endStroke();
      }
    },
    ...
    _displayCross: function () {
      ...
      this.cross.graphics
        .setStrokeStyle(this.canvasData.lineWidth)
        .beginStroke(this.canvasData.color)
        .moveTo(cross.x, cross.y)
        .lineTo(cross.x + cross.width, cross.y + cross.height)
        .endStroke();
      this.cross.graphics
        .setStrokeStyle(this.canvasData.lineWidth)
        .beginStroke(this.canvasData.color)
        .moveTo(cross.x + cross.width, cross.y)
        .lineTo(cross.x, cross.y + cross.height)
        .endStroke();
      this.drawAngles();
    ...
    _tick: function () {
      //console.log("in tick")
      var curr = Model.getPoint(
        this.currentStartPoint,
        this.cAngle * Maths.TO_RADIANS,
        Model.step,
      );
      //	console.log(curr)
      ...
      if (
        curr.x > Model.boundry.xMin &&
        curr.x < Model.boundry.xMax &&
        curr.y > Model.boundry.yMin &&
        curr.y < Model.boundry.yMax
      ) {
        if (!this.auto) {
          if (curr.y >= Model.waterBoundry) {
            ...
            if (!hit) {
              ...
              this.canvas.graphics
                ...
                .setStrokeStyle(this.canvasData.lineWidth)
                .beginStroke(this.canvasData.color)
                .moveTo(this.firstPoint.x, this.firstPoint.y)
                .lineTo(this.pointOnSurface.x, this.pointOnSurface.y)
                .endStroke();
              this.canvas.graphics
                .setStrokeStyle(this.canvasData.lineWidth)
                .beginStroke(this.canvasData.color)
                .moveTo(this.pointOnSurface.x, this.pointOnSurface.y)
